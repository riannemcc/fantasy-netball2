...
export const UserExPlayersTable = ({ players, currentUser, className }) => {
  ...
  const findUserPlayerByPosition = (position) => {
    const exPlayer = (currentUser.exPlayers || []).find(player => player.position === position)
    if (exPlayer) {
  ...
  return (
    <div className={className}>
      ...
      <table className="mb-8 shadow-lg bg-gray-100">
        ...
        <tbody>
          {currentUser.exPlayers && currentUser.exPlayers.map(({ playerId, dateAdded, dateRemoved, wasCaptain, wasViceCaptain }) => {
            const player = findPlayerById(playerId, players)
            return (
              <tr key={''}>
                <td className="border border-black px-4 py-2">
                  {player && player.name}
                  {wasCaptain ? <span class="w-6 bg-pink text-white rounded p-1 m-2">C</span> : ''}
                  {wasViceCaptain ? <span class="w-6 bg-pink text-white rounded p-1 m-2">VC</span> : ''}
                </td>
                <td className="border border-black px-4 py-2">
                  {player ? calculateExPlayerPoints(player, dateAdded, dateRemoved, wasCaptain, wasViceCaptain) : ' - '}
                </td>
              </tr>
            )
          })}
