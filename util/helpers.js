...
export function calculatePlayerPoints(player, startDate, endDate) {
  ...
  if (player && player.games && player.games.length > 0) {
    playerPoints = player.games
      .filter(game => {
        const gameStartDate = moment(game.startDateTime || DEFAULT_GAME_START_DATE)
        return gameStartDate.isSameOrAfter(startDate) && gameStartDate.isSameOrBefore(endDate)
      })
...
export function calculateExPlayerPoints(player, dateAdded, dateRemoved, wasCaptain, wasViceCaptain) {
...
export function calculateUserPoints(user, players) {
  let userPoints = 0
  if (user) {
    userPoints += (user.teamPlayers || [])
      .reduce((userPointsAcc, { playerId, dateAdded }) => {
    ...
    userPoints += (user.exPlayers || [])
      .reduce((userPointsAcc, { playerId, dateAdded, dateRemoved, wasCaptain, wasViceCaptain }) => {
        const player = findPlayerById(playerId, players)
        const playerPoints = calculateExPlayerPoints(player, dateAdded, dateRemoved, wasCaptain, wasViceCaptain)
        return userPointsAcc + playerPoints
      }, 0)
